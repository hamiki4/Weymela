...
|---|---|---|
| Session | `/auth/mode`, `/session` | `/session/sign-out`; `/development/session` only in explicitly enabled Development |
...
- Social verification is a test-provider adapter, not TikTok/YouTube/Instagram integration. Public identity and production sign-in adapters are intentionally unconfigured/fail-closed.
