...
The V3 backup identity is read-only. Apply the reviewed `database/grants/v3-backup.sql` only after target and backup-role checks; the migration owner separately runs `v3-migrator-defaults.sql` as itself, without new role membership or SUPERUSER. Then run `v3-verify.sql` with all four operational/runtime role names. The backup grant includes the complete current persisted V3 inventory, including sensitive authentication, device, legal, and financial records; protect the archive accordingly. Future SELECT defaults apply only to objects created by the V3 migration owner in `public` or `v3`. Reverify after each approved migration, particularly if another object creator is introduced. The backup identity must never be used for restore: restore is a separately authorized bootstrap/migration operation against an explicitly identified target.
