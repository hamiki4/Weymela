...
4. Execute verified `efbundle` only after target guard, reviewed grants/SQL and approval. **Current design-time factory does not read a deployment connection environment variable**: the EF bundle requires an explicit `--connection` override. Use `tools/ops/run-v3-migrations.py` as the fail-closed wrapper; it requires a password-free `Passfile` connection, requires `Username=weymela_v3_migrator`, and verifies both `current_user` and `session_user` before starting EF. The wrapper keeps the approved Npgsql semicolon-format connection string for the bundle, while deriving explicit `psql`/libpq host, port, database and username arguments plus `PGPASSFILE` for the identity probe; it never passes the Npgsql string as a `psql` argument. Preferred secure invocation supplies a password-free connection (exact new host/database/migrator) and Npgsql's protected external password-file mechanism; verify that mechanism with the built bundle in an isolated rehearsal. Do not pass a credential-bearing string in arguments, enable verbose logs or use existing Pilot/Production credentials. If password-file authentication cannot be verified, stop and approve a secure one-shot migrator configuration adapter; do not assume an env file overrides the factory. Bundle behavior is documented by [Microsoft EF Core](https://learn.microsoft.com/en-us/ef/core/managing-schemas/migrations/applying).
   Npgsql supports `Passfile=/protected/v3-migrator.pgpass` (or `PGPASSFILE`); see [Npgsql connection parameters](https://www.npgsql.org/doc/connection-string-parameters.html). A future password-free invocation is `WEYMELA_V3_MIGRATOR_CONNECTION='Host=weymela-v3-pilot-postgres;Database=weymela_v3_pilot;Username=weymela_v3_migrator;Passfile=/run/secrets/v3-migrator.pgpass;Include Error Detail=false' python3 tools/ops/run-v3-migrations.py /approved/v3-migrations/efbundle` inside the approved isolated maintenance context. No actual password appears in that argument. Exact host identity and secret-file0600 permissions still need verification; this is not a command authorized to execute now.
