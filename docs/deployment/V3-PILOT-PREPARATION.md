...
| Firebase access | Server verifier exists; live client adapter and trusted-user bootstrap are not finished | Implement/review existing-project sign-in and controlled role provisioning; no Development identities outside Development |
...
Required external inputs: approved GHCR digests, separate DB bootstrap/runtime/migrator/backup credentials, Firebase public project identity, cookie-protection private PFX/password/keyring, exact public origins, approved proxy/subnet, verified TLS edge, user/legal/pricing bootstrap. No FCM/private Firebase key is needed for current public-signature verification. No Android material belongs here.
