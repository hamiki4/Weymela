...
The verified external subject must map to an active `IdentityBinding`, exactly one trusted active `CommercePermission`, and a public workspace profile where required. JWT role claims and payload-selected roles are not authorization inputs. No public identity-binding or role-grant endpoint exists. Provisioning/revocation is a separately authorized administrative prerequisite; multiple memberships fail closed until secure workspace switching is designed.
...
The future Firebase client adapter must obtain a fresh ID token and POST it to `/api/auth/firebase/session`; it must not persist bearer tokens in localStorage or send role/Business selectors. The existing non-development sign-in screen remains an integration boundary, not a live sign-in claim.
