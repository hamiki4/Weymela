...
public enum EarningSource { ViewReward, SaleCommission, AuthorizedAdjustment }
public enum CashbackSource { VerifiedSale, AuthorizedAdjustment }
public enum PlatformRevenueSource { ViewRewardPlatformShare, SalePlatformShare, AuthorizedAdjustment }
...
public sealed class Promotion
{
    ...
    public string Title { get; private set; } public string Description { get; private set; } public PromotionType PromotionType { get; }
    ...
    public CreatorEligibilityCriteria Eligibility { get; } public DateTime StartDateUtc { get; } public DateTime EndDateUtc { get; } public PricingSnapshot PricingSnapshot { get; } public PromotionStatus Status { get; private set; } = PromotionStatus.Draft; public DateTime CreatedAtUtc { get; } public DateTime? PublishedAtUtc { get; private set; } public DateTime? ActivatedAtUtc { get; private set; } public DateTime? CompletedAtUtc { get; private set; } public long Version { get; private set; }
    public IReadOnlyList<CreatorAllocation> Allocations => allocations; public IReadOnlyList<DomainEvent> DomainEvents => events;
    public Promotion(Guid businessId, string title, string description, PromotionType type, Money totalBudget, CreatorEligibilityCriteria eligibility, DateTime start, DateTime end, PricingSnapshot pricing, DateTime createdAt) { if (string.IsNullOrWhiteSpace(title)) throw new ArgumentException("Title is required."); if (end <= start) throw new ArgumentException("End must follow start."); if (totalBudget.Amount <= 0) throw new ArgumentOutOfRangeException(nameof(totalBudget)); if (pricing.PromotionType != type) throw new ArgumentException("Pricing snapshot type mismatch."); if (pricing.MinimumPromotionBudget is { } minimum && totalBudget.Amount < minimum.Amount) throw new InvalidOperationException("Promotion budget is below the effective minimum for this Promotion type."); BusinessId = businessId; Title = title.Trim(); Description = description?.Trim() ?? ""; PromotionType = type; TotalBudget = totalBudget; ReservedBudget = Money.Zero(totalBudget.Currency); UsedBudget = Money.Zero(totalBudget.Currency); Eligibility = eligibility; StartDateUtc = start; EndDateUtc = end; PricingSnapshot = pricing; CreatedAtUtc = createdAt; }
    ...
    public CreatorAllocation Allocate(Guid creatorId, Money amount, DateTime at, Guid correlation) { Ensure(PromotionStatus.Active, PromotionStatus.Published, PromotionStatus.Funded); if (amount.Currency != TotalBudget.Currency || amount.Amount <= 0 || amount.Amount > UnallocatedBudget.Amount) throw new InvalidOperationException("Allocation exceeds unallocated Promotion budget."); var allocation = new CreatorAllocation(Id, creatorId, amount, at); allocations.Add(allocation); Version++; events.Add(new CreatorAllocationCreated(allocation.Id, amount, at, correlation)); return allocation; }
...
public sealed class CreatorApplication
{
    ...
    public Guid Id { get; } = Guid.NewGuid(); public Guid PromotionId { get; } public Guid CreatorId { get; } public string Message { get; } public string? ContentConcept { get; } public CreatorApplicationStatus Status { get; private set; } = CreatorApplicationStatus.Pending; public DateTime AppliedAtUtc { get; } public DateTime? ReviewedAtUtc { get; private set; } public Guid? ReviewedByBusinessUserId { get; private set; }
...
public sealed class CreatorAllocation
{
    ...
    public long Version { get; private set; }
    public Guid Id { get; } = Guid.NewGuid(); public Guid PromotionId { get; } public Guid CreatorId { get; } public Money OriginalAllocation { get; private set; } public Money UsedAmount { get; private set; } public Money RemainingAmount => OriginalAllocation.Subtract(UsedAmount); public CreatorAllocationStatus Status { get; private set; } = CreatorAllocationStatus.Active; public DateTime ApprovedAtUtc { get; } public DateTime? ActivatedAtUtc { get; private set; } public DateTime? CompletedAtUtc { get; private set; }
    public CreatorAllocation(Guid promotionId, Guid creatorId, Money amount, DateTime at) { if (amount.Amount <= 0) throw new ArgumentOutOfRangeException(nameof(amount)); PromotionId = promotionId; CreatorId = creatorId; OriginalAllocation = amount; UsedAmount = Money.Zero(amount.Currency); ApprovedAtUtc = at; }
...
public sealed record CreatorEarningEntry(Guid Id, Guid CreatorId, Guid? PromotionId, EarningSource Source, Money Amount, DateTime CreatedAtUtc, Guid CorrelationId);
...
public sealed record PlatformRevenueEntry(Guid Id, Guid? PromotionId, PlatformRevenueSource Source, Money Amount, RevenueStatus Status, DateTime CreatedAtUtc, Guid CorrelationId);
