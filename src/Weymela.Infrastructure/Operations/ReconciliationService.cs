...
public sealed class ReconciliationService(WeymelaDbContext db)
{
    ...
    public async Task<IReadOnlyList<ReconciliationIssue>> CheckAsync(Actor actor, CancellationToken ct)
    {
        ...
        return await db.Database.SqlQueryRaw<ReconciliationIssue>("""
            WITH entries AS (
              SELECT j."BusinessId", j."PromotionId", j."CreatorId", j."CustomerId", l."Account", l."Type", l."Amount"
              ...
              UNION ALL SELECT 'BusinessReserved', w."BusinessId",
                coalesce((SELECT sum(CASE WHEN e."Type"='Credit' THEN e."Amount" ELSE -e."Amount" END) FROM entries e WHERE e."BusinessId"=w."BusinessId" AND e."Account" IN ('CampaignUnallocatedReserve','CreatorAllocatedReserve')),0), w."ReservedBalance" FROM v3."BusinessWallets" w
