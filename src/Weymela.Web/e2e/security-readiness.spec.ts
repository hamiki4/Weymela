...
for (const width of [375, 390, 393, 430, 768, 1366, 1440, 1920])
  test(`operational error, offline and inbox layouts at ${width}px`, async ({
    page,
    context,
  }) => {
    ...
    await layout(page);
    await screenshot(page, `${width}-notifications`);
    await context.setOffline(true);
    await expect(page.getByRole("alert")).toContainText("Nothing is queued");
    await layout(page);
    await screenshot(page, `${width}-offline-workspace`);
    ...
    await expect(
      page.getByRole("heading", {
        name: "This workspace isn’t available to your role",
      }),
    ).toBeVisible();
    await layout(page);
    await screenshot(page, `${width}-unauthorized`);
  });

test("durable in-app notification read state survives reload", async ({
  page,
  context,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "0 unread", exact: true }),
  ).toBeVisible();
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "0 unread", exact: true }),
...
test("service worker caches only safe offline assets and never financial responses", async ({
  page,
  context,
}) => {
  await login(context, "business");
  ...
  await screenshot(page, "pwa-safe-offline-document");
  await context.setOffline(false);
});

test("production assets have safe headers and release-specific update identity", async ({
  request,
}) => {
  ...
  expect(await sw.text()).not.toContain("__BUILD_ID__");
  const manifest = await (await request.get("/manifest.webmanifest")).json();
  for (const size of [192, 512]) {
    const icon = manifest.icons.find(
      (x: { sizes: string }) => x.sizes === `${size}x${size}`,
    );
    expect(icon).toBeTruthy();
    expect((await request.get(icon.src)).status()).toBe(200);
  }
  expect(
    manifest.icons.some((icon: { src: string }) => icon.src.endsWith(".svg")),
  ).toBe(false);
  for (const icon of [
    "/favicon-16.png",
    "/favicon-32.png",
    "/brand/weymela-mark.png",
    "/brand/weymela-wordmark.png",
  ])
