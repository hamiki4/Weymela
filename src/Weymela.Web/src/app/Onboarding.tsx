...
function publicRole(role: Enrollment["role"]): OnboardingRole | null {
  return role === 3 || role === "Customer"
    ? "Customer"
    : role === 2 || role === "Creator"
      ? "Creator"
      : role === 1 || role === "Business"
        ? "Business"
        : null;
}
...
export function Onboarding() {
  ...
  const submitCustomer = () =>
    void action.run(async (key) => {
      const terms = legal.data?.documents.find(
        (document) => document.kind === "TermsOfService",
      );
      const privacy = legal.data?.documents.find(
        (document) => document.kind === "PrivacyPolicy",
      );
  ...
  return (
    <main className="main-content page-shell section-kicker-space onboarding-page">
      <PageHeader
        eyebrow="Your Weymela account"
        title="How do you want to use Weymela?"
        action={
      ...
      <Resource resource={status}>
        {(data) => {
          const pending = new Set(
            data.profiles
              .filter((item) => item.status === "Pending" || item.status === 0)
          ...
          const unavailable = new Set(
            data.profiles
              .filter((item) => {
                ...
                return (
                  (item.status === "Approved" || item.status === 1) &&
                  activeRole !== null &&
                  ...
                  <div className="stack-list">
                    {data.profiles.map((item) => (
                      <div className="amount-row" key={item.id}>
                        <div>
                          <strong>
                            {item.displayName || publicRole(item.role) || "Profile"} —{" "}
                            {unavailable.has(publicRole(item.role)) &&
                            (item.status === "Approved" || item.status === 1)
                              ? "Unavailable"
                              : statusLabel(item.status)}
                          </strong>
                          {item.decisionReason && <small>{item.decisionReason}</small>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <Section title="Choose a profile">
                <div className="content-grid profile-choice-grid">
                  {choices.map(([choice, description, actionLabel, className]) => {
                    const state = approved.has(choice)
                      ? "Already added"
                      : pending.has(choice)
                        ? "Pending"
                        : unavailable.has(choice)
                          ? "Unavailable"
                          : actionLabel;
                    ...
                    return (
                      <button
                        key={choice}
                        className={`profile-choice ${className}${role === choice ? " selected" : ""}`}
                        type="button"
                        aria-label={
                          disabled
                            ? `${choice} — ${state}`
                  ...
                  {approved.size === 3 && (
                    <p className="fine-print">
                      All three profiles are active. Switch profile to use another one.
                    </p>
                  )}
                </div>
                ...
                {role === "Customer" && (
                  ...
                  >
                    <Resource resource={legal}>
                      {(documents) =>
                        documents.available ? (
                          <form
                            className="form-grid onboarding-form customer-onboarding-form"
                            onSubmit={(event) => {
                              event.preventDefault();
                              submitCustomer();
                            }}
                          >
                            <Field label="Preferred name" wide>
                              <input
                                required
                                autoComplete="name"
                                maxLength={120}
                          ...
                          <div className="onboarding-unavailable" role="status">
                            <Empty
                              icon="lock"
                              title="Customer setup isn't available yet."
                              message="Required terms and privacy information have not been published."
                              action={
                ...
                {(role === "Creator" || role === "Business") && (
                  <RoleOnboardingShell
                    ...
                    description={
                      role === "Creator"
                        ? "Your Creator profile setup will be available soon."
                        : "Your Business profile setup will be available soon."
                    }
                  >
                    <div className="onboarding-coming-soon" role="status">
                      <p>There’s nothing you need to enter yet.</p>
                      <Button variant="secondary" onClick={cancel}>
                        Back
                      </Button>
                    </div>
