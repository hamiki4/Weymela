...
public sealed class CampaignFlowHttpTests(PostgresFixture postgres)
{
    [Fact] public async Task Concurrent_approval_and_rejection_cannot_leave_a_rejected_applicant_with_a_budget()
    {
        ...
        var row=(await c.GetJson("/api/business/campaigns")).AsArray().Single(r=>r!["type"]!.GetValue<string>()=="ViewPlusCommission")!;
    ...
    private static object Draft(decimal budget=500)=>new{title="HTTP Campaign",description="A real persisted Campaign",type="ViewPlusCommission",campaignBudget=budget,requirements="Original content",category="Food",region="Addis Ababa",minimumVerifiedFollowers=1000,startUtc=DateTime.UtcNow.AddMinutes(-1),endUtc=DateTime.UtcNow.AddDays(5)};
    ...
    [Fact] public async Task Future_settings_do_not_apply_early_and_saved_versions_are_authoritative()
    {
        ...
        var campaign=(await business.GetJson("/api/business/campaigns")).AsArray().Single(r=>r!["type"]!.GetValue<string>()=="ViewOnly")!;Assert.Equal(300,(await business.GetJson($"/api/business/campaigns/{campaign["id"]!.GetValue<string>()}"))["pricing"]!["businessPays"]!.GetValue<decimal>());
    }
    ...
    {
        await using var f=await ApiFixture.CreateAsync(postgres);using var c=await f.Login("admin");var queue=await c.GetJson("/api/admin/payouts");var creator=queue["creators"]![0]!;var prepared=await c.PostJson($"/api/admin/payouts/Creator/{creator["subjectId"]!.GetValue<string>()}/prepare",new{});await c.PostJson($"/api/admin/payouts/{prepared["id"]!.GetValue<string>()}/paid",new{reference="test-payment-1"},"pay");await c.PostJson($"/api/admin/payouts/{prepared["id"]!.GetValue<string>()}/paid",new{reference="test-payment-1"},"pay");using var own=await f.Login("creator");Assert.Equal(370,(await own.GetJson("/api/creator/earnings"))["availableEarnings"]!.GetValue<decimal>());await c.PostJson("/api/admin/platform/settlements",new{amount=100,reference="test-settlement"});Assert.Equal(410,(await c.GetJson("/api/admin/platform"))["unsettled"]!["amount"]!.GetValue<decimal>());
    }
}
