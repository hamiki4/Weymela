...
public sealed class WorkspaceHttpTests(PostgresFixture postgres)
{
    ...
    {await using var f=await ApiFixture.CreateAsync(postgres);using var c=await f.Login("creator");var p=await c.GetJson("/api/creator/pricing");Assert.Equal(500,p["minimumToCashOut"]!.GetValue<decimal>());Assert.Equal(4.5m,p["rows"]![1]!["saleCommissionPercent"]!.GetValue<decimal>());foreach(var forbidden in new[]{"businessPays","platform","cashback"})Assert.DoesNotContain(forbidden,p.ToJsonString());}
