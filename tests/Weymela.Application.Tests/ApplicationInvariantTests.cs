...
public sealed class ApplicationInvariantTests
{
    ...
    private static Promotion TestPromotion(Guid businessId) => new(businessId, "Campaign", "", PromotionType.ViewOnly, new Money(1000), new(null, null, null, null), Now, Now.AddDays(1), new PricingSnapshot(PromotionType.ViewOnly, 1000, new Money(300), new Money(200), new Money(100), 0, 0, 0, Now, Guid.NewGuid()), Now);
    ...
    private sealed class Pricing : IFinancialConfigurationResolver { public Task<PricingSnapshot> ResolveAsync(PromotionType type, DateTime at, CancellationToken ct) => Task.FromResult(new PricingSnapshot(type, 1, new Money(1), new Money(1), new Money(0), 0, 0, 0, at, Guid.NewGuid())); }
