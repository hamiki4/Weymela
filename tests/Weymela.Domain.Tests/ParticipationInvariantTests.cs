...
public sealed class ParticipationInvariantTests
{
 ...
 private static Promotion Published(){var s=new PricingSnapshot(PromotionType.ViewOnly,1000,new Money(300),new Money(200),new Money(100),0,0,0,Now,Guid.NewGuid());var p=new Promotion(Guid.NewGuid(),"Campaign","",PromotionType.ViewOnly,new Money(6000),new(null,null,"ET",null),Now,Now.AddDays(1),s,Now);var w=new BusinessWallet(p.BusinessId);w.CreditDeposit(new Money(6000),Now,Guid.NewGuid());p.Fund(w,Now,Guid.NewGuid());p.Publish(Now,Guid.NewGuid());return p;}
 ...
 [Fact] public void Draft_campaign_rejects_application(){var s=new PricingSnapshot(PromotionType.ViewOnly,1,new Money(1),new Money(1),new Money(1),0,0,0,Now,Guid.NewGuid());var p=new Promotion(Guid.NewGuid(),"x","",PromotionType.ViewOnly,new Money(1),new(null,null,null,null),Now,Now.AddDays(1),s,Now);Assert.Throws<InvalidOperationException>(()=>new CreatorApplication(p.Id,Guid.NewGuid(),null,null,p,Now,Guid.NewGuid()));}
