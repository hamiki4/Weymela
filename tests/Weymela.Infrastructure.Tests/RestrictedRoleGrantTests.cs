...
public sealed class RestrictedRoleGrantTests(PostgresFixture fixture)
{
    ...
    public async Task Runtime_grants_are_exact_and_real_flows_succeed_under_restricted_roles()
    {
        ...
        await AssertInsufficientPrivilegeAsync(apiConnection,
            "SELECT * FROM v3.\"RetiredSupportSessions\"",
            "API must not read retired support-session security evidence");
        await AssertInsufficientPrivilegeAsync(workerConnection,
            "SELECT * FROM v3.\"RetiredSupportSessions\"",
            "Worker must not read retired support-session security evidence");
        ...
        FileInfo verifyScript)
    {
        ...
        foreach (var table in new[] { "AuthIdentifiers", "EmailAuthChallenges", "PasswordCredentials", "AuthorizedDevices",
                     "DeviceSessions", "ProductHandoffTransactions", "LegalAcceptances", "CustomerProfiles", "FinancialJournals" })
